...
from bot.helper.ext_utils.bot_utils import get_user_tasks, getdailytasks, sync_to_async, get_telegraph_list, get_readable_file_size, get_readable_time
...
async def task_utils(message):
    ...
    if (bmax_tasks := config_dict['BOT_MAX_TASKS']) and len(download_dict) >= bmax_tasks:
        msg.append(f"⭕️ ربات به حد مجاز استارت تسک رسیده است ⭕️")
    if (maxtask := config_dict['USER_MAX_TASKS']) and await get_user_tasks(message.from_user.id, maxtask):
        msg.append(f"⭕️ شما تنها قادر به استارت {maxtask} تسک هستید ⭕️")
    return msg, button

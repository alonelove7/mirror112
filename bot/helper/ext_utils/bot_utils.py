...
def get_readable_message():
    ...
    for download in list(download_dict.values())[STATUS_START:STATUS_LIMIT+STATUS_START]:
        ...
        msg += BotTheme('STATUS_NAME', Name="♻️ تسک درحال تکمیل است" if config_dict['SAFE_MODE'] and elapsed >= config_dict['STATUS_UPDATE_INTERVAL'] else escape(f'{download.name()}'))
...
async def get_stats(event, key="home"):
    ...
    if key == "home":
        btns = ButtonMaker()
        btns.ibutton('وضعیت ربات 🤖', f'wzmlx {user_id} stats stbot')
        btns.ibutton('👾 وضعیت سیستم', f'wzmlx {user_id} stats stsys')
        btns.ibutton('وضعیت سورس 📦', f'wzmlx {user_id} stats strepo')
        btns.ibutton('📈 لیست محدودیت ها', f'wzmlx {user_id} stats botlimits')
        msg = "❇️ به ربات کینگ لیچ خوش آمدید\n\n🆔@King_Network7"
    elif key == "stbot":
        ...
        official_v = (await cmd_exec(f"curl -o latestversion.py https://raw.githubusercontent.com/{config_dict['UPSTREAM_BRANCH']}/bot/version.py -s && python3 latestversion.py && rm latestversion.py", True))[0]
    ...
    try:
        bot_cmds = [
            ...
            BotCommand(
                ...
                'Get detailed help about the KingLeech',
            ),
            BotCommand(

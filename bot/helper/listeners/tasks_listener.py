...
class MirrorLeechListener:
    ...
    async def onUploadComplete(self, link, size, files, folders, mime_type, name, rclonePath='', private=False):
        ...
        msg = BotTheme('NAME', Name="✅ عملیات با موفقیت به پایان رسید"if config_dict['SAFE_MODE'] and self.isSuperGroup else escape(name))
    ...
    async def onDownloadError(self, error, button=None):
        async with download_dict_lock:
            ...
            if self.sameDir and self.uid in self.sameDir['tasks']:
                self.sameDir['tasks'].remove(self.uid)
                self.sameDir['total'] -= 1
        msg = f'''❌  دانلود شما متوقف شد
👤 تسک کاربر: {self.tag}
〽️ حالت: <code>{self.upload_details['mode']}</code>
⏱ زمان سپری شده: <code>{get_readable_time(time() - self.message.date.timestamp())}</code>
‼️ علت توقف: <code>{escape(error)}</code>
